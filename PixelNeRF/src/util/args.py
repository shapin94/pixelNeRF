...
def parse_args(
    callback=None,
    training=False,
    default_conf="conf/exp/dtu.conf",#"conf/default_mv.conf",
    default_expname="example", # rendered video will be saved in 'visuals/default_expname/'
    ...
    parser.add_argument( # specify gpu id to use
        "--gpu_id", type=str, default="0", help="GPU(s) to use, space delimited"
    )
    parser.add_argument( # specify output folder name
        "--name", "-n", type=str, default=default_expname, help="experiment name"
    )
    parser.add_argument( # choose one which corresponds to the dataset you would to use
        "--dataset_format",
        "-F",
        type=str,
        default="dvr_dtu",
    ...
    parser.add_argument( # specify path to get required dataset
        "--datadir", "-D", type=str, default="data/rs_dtu_4", help="Dataset directory"
    )
    parser.add_argument(
