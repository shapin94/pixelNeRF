...
# environment.yml does not contains dotmap. should be installed separatelly. pip install dotmap

# Parallel Rendering Wrapper. 
# call NeRFRenderer
class _RenderWrapper(torch.nn.Module):
    ...
    def forward(self, rays, want_weights=False):
        ...
        outputs = self.renderer( # start rendering using given inputs
            self.net, rays, want_weights=want_weights and not self.simple_output
        )
        if self.simple_output:
            if self.renderer.using_fine:
                rgb = outputs.fine.rgb # get final composite result. coarse -> fine result
...
######### differentiable Renderer of NeRF
# note that NeRF volume rendering uses coarse-to-fine sampling 
class NeRFRenderer(torch.nn.Module):
    ...
    def __init__( # default setting - coarse: 64, fine: 128 
        self,
        n_coarse=128,
        n_fine=0,
        n_fine_depth=0,
    ...
    def sample_coarse(self, rays):
        ...
        :param rays ray [origins (3), directions (3), near (1), far (1)] (B, 8) # 8 dimensional inputs. target camera view center and corresponding direction + near/far
        :return (B, Kc)
        """
        device = rays.device
        near, far = rays[:, -2:-1], rays[:, -1:]  # (B, 1)

        step = 1.0 / self.n_coarse # define step size - ratio - based on number of coarse sampling size 
        B = rays.shape[0]
        z_steps = torch.linspace(0, 1 - step, self.n_coarse, device=device)  # (Kc)
        z_steps = z_steps.unsqueeze(0).repeat(B, 1)  # (B, Kc) # define N_c location in 0-1 linear space 
    ...
    # alpha compositing after coarse / fine sampling 
    def composite(self, model, rays, z_samp, coarse=True, sb=0):
        ...
        with profiler.record_function("renderer_composite"):
            ...
            # (B, K, 3)
            points = rays[:, None, :3] + z_samp.unsqueeze(2) * rays[:, None, 3:6] # camera center + sampling_location*direction
            points = points.reshape(-1, 3)  # (B*K, 3) = 64*50000, 3
            ...
            if use_viewdirs:
                ...
                for pnts, dirs in zip(split_points, split_viewdirs):
                    val_all.append(model(pnts, coarse=coarse, viewdirs=dirs)) # input view direciton + points for each ray sample # val all has rgb, density val of 64 Nc
            else:
    ...
    def forward(
        ...
        """
        :model nerf model, should return (SB, B, (r, g, b, sigma)) # SB for object batch 
        ...
        with profiler.record_function("renderer_forward"):
            ...
            rays = rays.reshape(-1, 8)  # (SB * B, 8) # flatten given ray samples to process independently 

            z_coarse = self.sample_coarse(rays)  # (B, Kc) = 50000, 64. N_c locations
            ...
            if self.using_fine: # similar operation for fine sampling step 
                all_samps = [z_coarse]
    ...
    # multi-GPU binder for parallel training
    # output wrapped parallel binding of given network 
