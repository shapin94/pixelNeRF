...
class PixelNeRFNet(torch.nn.Module):
    ...
    # can decide whether to use image feature prior or not
    # can specify other options include positional encoding / canonical space recentering etc
    def __init__(self, conf, stop_encoder_grad=False):
        ...
        # define latent code size (512) if using image feautre 
        d_latent = self.encoder.latent_size if self.use_encoder else 0
        d_in = 3 if self.use_xyz else 1 # dimension of input is x,y,z coordinate 
    ...
    def encode(self, images, poses, focal, z_bounds=None, c=None):
        """
        :param images (NS, 3, H, W) NS - Number of Samples 
        ...
        """
        self.num_objs = images.size(0) # number of object patch
        if len(images.shape) == 5:
            assert len(poses.shape) == 4
            assert poses.size(1) == images.size(
                1
            )  # Be consistent with NS = num input views
            self.num_views_per_obj = images.size(1) # number of input source
            images = images.reshape(-1, *images.shape[2:])
            poses = poses.reshape(-1, 4, 4)
        else:
            self.num_views_per_obj = 1

        self.encoder(images) # extract corresponding latent feature vector 
        rot = poses[:, :3, :3].transpose(1, 2)  # (B, 3, 3)
        trans = -torch.bmm(rot, poses[:, :3, 3:])  # (B, 3, 1)
        self.poses = torch.cat((rot, trans), dim=-1)  # (B, 3, 4) # transformation matrix to each camera view space 
    ...
    # generate r,g,b, density from points and view direction. output will be used for alpha compositing 
    def forward(self, xyz, coarse=True, viewdirs=None, far=False):
        ...
        with profiler.record_function("model_inference"):
            SB, B, _ = xyz.shape
            NS = self.num_views_per_obj # number of source images
            ############################################################## query point into camera space 
            # Transform query points into the camera spaces of the input views
            xyz = repeat_interleave(xyz, NS)  # (SB*NS, B, 3)
            xyz_rot = torch.matmul(self.poses[:, None, :3, :3], xyz.unsqueeze(-1))[
                ..., 0
            ]
            xyz = xyz_rot + self.poses[:, None, :3, 3] # rotation + translation
            ...
            if self.d_in > 0:
                ...
                if self.use_code and self.use_code_viewdirs: # 2nd positional encoding 
                    # Positional encoding (with viewdirs)
                    z_feature = self.code(z_feature)

                mlp_input = z_feature

            if self.use_encoder: # residual feature (image feature priors)
                ...
                if self.d_in == 0:
                    ...
                    mlp_input = latent
                else: # use residual image feature 
            ...
            # Run main NeRF network # mlp_sampling  & image feature concat are all done 
            if coarse or self.mlp_fine is None: 
                mlp_output = self.mlp_coarse( # multiview feature aggregation
                    mlp_input,
                    combine_inner_dims=(self.num_views_per_obj, B),
                    combine_index=combine_index,
                    dim_size=dim_size,
                ) # mlp output : rgb, sigma
            else:
        ...
        return output # output shape : SB, Sampled_Ray, 4
