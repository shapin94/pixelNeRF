...
class SpatialEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        if self.feature_scale != 1.0: #if feature volume scale doesn't meet the exact image size 
            x = F.interpolate( # bilinear interpolation to sample corresponding W(pi(x))
                x,
