...
# Resnet Blocks # multiview input features aggregation 
...
class ResnetFC(nn.Module):
    ...
    def forward(self, zx, combine_inner_dims=(1,), combine_index=None, dim_size=None):
        """
        :param zx (..., d_latent + d_in) # positional encoded viewing direction + ray + latent size 512
        ...
        with profiler.record_function("resnetfc_infer"):
            ...
            if self.d_latent > 0: # distinguish positional encded input and latent 
                z = zx[..., : self.d_latent]
                x = zx[..., self.d_latent :]
            else:
                x = zx
            if self.d_in > 0:
                x = self.lin_in(x) # 42 to 512
            else:
                x = torch.zeros(self.d_hidden, device=zx.device)

            for blkid in range(self.n_blocks):
                if blkid == self.combine_layer: # default: combine feautres when blkid == 3
                    ...
                    #      )
                    #  else:
                    x = util.combine_interleaved( # aggregate features using average operation
