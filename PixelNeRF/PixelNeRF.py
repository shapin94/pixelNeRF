...
# case: DTU MVS Data
# To do list 
# 1. download DTU dataset / create subfolder named 'data' / extract data to it as 'data/rs_dtu_4'
#    download link:  https://drive.google.com/drive/folders/1PsT3uKwqHHD2bEEHkIXB99AlIjtmrEiR?usp=sharing
# 2. download pretrained weights / create subfolder named 'checkpoints' / extract weights to it as 'checkpoints/dtu'
# 3. create subfolder named 'visuals' for output
# 4. modify arguments properly in gen_video.py and src/args.py
# 5. run / check results in visuals/example/

def extra_args(parser):
    parser.add_argument( # specify scene id to synthesize
        "--subset", "-S", type=int, default=3, help="Subset in data to use"
    )
    parser.add_argument(
        "--split",
        type=str,
        default="val", # specify data split to use 
    ...
    parser.add_argument(
        ...
        "-P",
        type=str,
        default="22 25 28", # pick number of source views to use
    ...
    parser.add_argument( # 0.25 -> 1.0 to render at full resolution (quite slow, not recommended)
        "--scale", type=float, default=0.25, help="Video scale relative to input size"
    )
    parser.add_argument(
        "--radius",
        type=float,
...
# get desired dataset specified by the arguments 
...
# decide rendering view variance of synthesized scene. (decide where to look)
if dtu_format:
    ...
    print("Using default (360 loop) camera trajectory") # if not using IDR-like camera trajectory, using 360 loop trajectory like NeRF
    if args.radius == 0.0:
...
render_rays = util.gen_rays(
    render_poses, # generate / sample rays based on render poses. 
    W,
    H,
    focal * args.scale,
    z_near,
...
# (NV, H, W, 8) # 8 dim: each for cam_centers(3) + ray_direction(3) + near(1) + far(1)

focal = focal.to(device=device)

source = torch.tensor(list(map(int, args.source.split())), dtype=torch.long)
NS = len(source)
random_source = NS == 1 and source[0] == -1 #random source option if it's not manually defined
assert not (source >= NV).any()

if renderer.n_coarse < 64: # least sampling num handling
...
with torch.no_grad():
    ...
    if random_source:
        src_view = torch.randint(0, NV, (1,)) # in case of random source 
    else:
        src_view = source

    net.encode( # encode image feature priors 
    ...
    for rays in tqdm.tqdm(
        ...
        rgb, _depth = render_par(rays[None]) # get final composited result 
        all_rgb_fine.append(rgb[0])
    _depth = None
    rgb_fine = torch.cat(all_rgb_fine)
    # rgb_fine (V*H*W, 3)

    frames = rgb_fine.view(-1, H, W, 3) # 48, 75, 100, 3
...
imageio.mimwrite( # save frames as video format
